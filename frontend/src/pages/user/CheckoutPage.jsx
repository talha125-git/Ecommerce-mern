...
export default function CheckoutPage() {
  ...
  const loadProfileDefaults = () => {
    ...
    if (loggedStatus) {
      try {
        ...
        const emailKey = savedUser.email ? `user_profile_${savedUser.email.toLowerCase()}` : null;
        const profileSavedObj = emailKey ? JSON.parse(localStorage.getItem(emailKey) || "{}") : {};

        const fullName = savedUser.name || profileSavedObj.name || "";
        const email = savedUser.email || profileSavedObj.email || "";
        const phone = savedUser.phone || profileSavedObj.phone || "";
  ...
  useEffect(() => {
    ...
    const handleProfileUpdated = () => loadProfileDefaults();
    window.addEventListener("profile-updated", handleProfileUpdated);
    return () => window.removeEventListener("profile-updated", handleProfileUpdated);
  }, []);

  // Load Pakistan cities on mount
  useEffect(() => {
            ...
            <CardHeader className="pb-4 flex flex-row items-center justify-between">
              ...
              {hasAutofilled && (
                <span className="inline-flex items-center gap-1 text-[11px] font-extrabold text-emerald-700 bg-emerald-50 border border-emerald-200 px-2.5 py-1 rounded-full">
                  <Sparkles className="w-3.5 h-3.5 text-emerald-500" /> Auto-filled from Profile
                </span>
              )}
            </CardHeader>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      readOnly
                      value="Pakistan 🇵🇰"
                      className="w-full pl-9 pr-3 py-2.5 bg-muted/60 border border-border rounded-xl font-bold cursor-not-allowed text-xs text-foreground"
                    />
                  </div>
                </div>
              </div>
